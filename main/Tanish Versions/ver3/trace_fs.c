...
int main(int argc, char **argv)
{
    ...
    if (pinned_fd >= 0) {
        ...
        trim(buf);
        if (buf[0]=='r'||buf[0]=='R'){ reuse=true; close(pinned_fd); }
        else { close(pinned_fd); unlink(pin_path); }
    }
    ...
    if (!reuse) {
        ...
        if (skel->maps.synthetic_fs) {
            if (bpf_map__pin(skel->maps.synthetic_fs, pin_path) < 0)
                fprintf(stderr, "Failed to pin synthetic_fs map\n");
            else
    ...
    if (reuse) {
        ...
        int skel_map_fd = bpf_map__fd(skel->maps.synthetic_fs);
        if (pinned_fd2 >= 0 && skel_map_fd >= 0) {
            char key[MAX_PATH] = {};
            char next[MAX_PATH] = {};
            ...
            if (bpf_map_get_next_key(pinned_fd2, NULL, key) == 0) {
                while (1) {
                    /* read value from pinned kernel map */
                    if (bpf_map_lookup_elem(pinned_fd2, key, &blk) == 0) {
                        /* write same value into the map used by the loaded/skel program */
            ...
            close(pinned_fd2); /* close temporary handle */
        } else {
            fprintf(stderr, "copy pinned->skel: invalid fds pinned=%d skel_map=%d\n",
                    pinned_fd2, skel_map_fd);
        }
    }
        ...
        if (select(STDIN_FILENO+1,&fds,NULL,NULL,&tv)>0 && FD_ISSET(STDIN_FILENO,&fds)){
            ...
            if(strcmp(line,"q")==0) break;
                ...
                if(fd<0){ fprintf(stderr,"open failed\n"); }
                    ...
                    char key[MAX_PATH]={}; strncpy(key,path,sizeof(key)-1);
                    if(bpf_map_lookup_elem(synthetic_map_fd,key,&blk)!=0){
                        memset(&blk,0,sizeof(blk));
                        bpf_map_update_elem(synthetic_map_fd,key,&blk,BPF_ANY);
                    }
                ...
                if(fd_to_path_map_fd>=0) bpf_map_delete_elem(fd_to_path_map_fd,&fd);
                ...
                buf[n] = '\0';  // BPF has already overwritten this memory
                printf("%s", buf);
            }
    ...
    if(!(buf[0]=='y'||buf[0]=='Y')) {
        ...
        else printf("Failed to delete pinned map\n");
    } else printf("Pinned map kept at %s\nExiting.\n",pin_path);
