...
int trace_read_exit(struct trace_event_raw_sys_exit *ctx) {
    ...
    /* copy fd to stack (can't use pointer-to-map-value directly as lookup key) */
    int fd = info->fd;
    ...
    /* lookup synthetic_fs using the stack key */
    struct fake_file_block *blk = bpf_map_lookup_elem(&synthetic_fs, key);
    if (!blk) goto cleanup;

    /* determine copy length (unsigned, clamped) */
    size_t copy_len = (size_t)ctx->ret;
    if (copy_len == 0) goto cleanup;
    if (copy_len > sizeof(blk->data)) copy_len = sizeof(blk->data);
    if (!info->buf_ptr) goto cleanup;

    bpf_printk("trace_read_exit pid=%d fd=%d copy_len=%d key=%s\n", (u32)(pid_tgid >> 32), fd, (int)copy_len, key);

    /* overwrite user buffer with synthetic content */
