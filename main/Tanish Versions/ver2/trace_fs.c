...
int main(int argc, char **argv)
{
    ...
    if (reuse) {
        ...
        if (synthetic_map_fd < 0) { fprintf(stderr,"Failed to open pinned synthetic map\n"); goto cleanup; }
        fd_to_path_map_fd = bpf_map__fd(skel->maps.fd_to_path);
        if (fd_to_path_map_fd < 0){ fprintf(stderr,"Failed to get fd_to_path map fd\n"); goto cleanup; }
        ...
        if (synthetic_map_fd < 0){ fprintf(stderr,"Failed to get synthetic map fd\n"); goto cleanup; }
        fd_to_path_map_fd = bpf_map__fd(skel->maps.fd_to_path);
        if (fd_to_path_map_fd < 0){ fprintf(stderr,"Failed to get fd_to_path map fd\n"); goto cleanup; }
    }
        ...
        if (select(STDIN_FILENO+1,&fds,NULL,NULL,&tv)>0 && FD_ISSET(STDIN_FILENO,&fds)){
            ...
            if(strcmp(line,"q")==0) break;
                ...
                if(fd<0){ fprintf(stderr,"open failed\n"); }
                    ...
                    // Add to fd_to_path BPF map
                    if(fd_to_path_map_fd>=0)
                        if(bpf_map_update_elem(fd_to_path_map_fd,&fd,path,BPF_ANY)!=0)
                            fprintf(stderr,"Failed to update fd_to_path map\n");
