...
def cmd_write(path, content, append=False):
    ...
    os.makedirs(os.path.dirname(abspath), exist_ok=True)
    with open(abspath, "a" if append else "w") as fh:
        fh.write(content)
    add_file_entry_for_path(abspath)
    print(f"Wrote to {abspath}")
...
def cmd_delete(path):
    abspath = os.path.normpath(os.path.abspath(path))
    if not os.path.exists(abspath):
        print("Path does not exist on disk. Removing from view if present.")
    else:
        if os.path.isfile(abspath):
            os.remove(abspath)
        elif os.path.isdir(abspath):
            shutil.rmtree(abspath)  # deletes folder + all contents
    # remove from in-memory state
    with state_lock:
        keys_to_remove = [k for k in file_index if k.startswith(abspath)]
        for k in keys_to_remove:
            del file_index[k]
